...
| BUG-013 | Onboarding appearance step is not scrollable, which traps the user on the theme screen | Onboarding / appearance step scrolling | `app/src/main/java/com/easyui/core/MainActivity.kt`, `copilot_session.md` | `./gradlew assembleDebug` | locally verified |
| BUG-014 | Need a secret Home entry point to reopen onboarding, triggered by 5 taps on the clock; status strip should sit above the clock | Home screen / hidden onboarding entry / top status layout | `app/src/main/java/com/easyui/core/MainActivity.kt`, `copilot_session.md` | `./gradlew assembleDebug` | locally verified |
